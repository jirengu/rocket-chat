...
var require = meteorInstall({"node_modules":{"meteor":{"rocketchat:ldap":{"server":{"ldap.js":["babel-runtime/helpers/classCallCheck",function(require){
...
}],"sync.js":["babel-runtime/helpers/typeof",function(require){
				...
				if (_.size(userData)) {                                                                                            // 111
					return {                                                                                                          // 112
						v: userData                                                                                                      // 112
                                                                                                                       ...
                                                                                                                       //
			if ((typeof _ret === "undefined" ? "undefined" : (0, _typeof3.default)(_ret)) === "object") return _ret.v;          // 65
		}                                                                                                                    // 114
	}                                                                                                                     // 115
                                                                                                                       //
...
syncUserData = function () {                                                                                           // 118
	function syncUserData(user, ldapUser) {                                                                               // 118
		...
		if (user && user._id && userData) {                                                                                  // 124
			Meteor.users.update(user._id, {                                                                                     // 125
				$set: userData                                                                                                     // 125
			});                                                                                                                 // 125
			user = Meteor.users.findOne({                                                                                       // 126
				_id: user._id                                                                                                      // 126
			});                                                                                                                 // 126
			...
			if (avatar) {                                                                                                       // 140
				...
				ws.on('end', Meteor.bindEnvironment(function () {                                                                  // 145
					Meteor.setTimeout(function () {                                                                                   // 146
						...
						RocketChat.Notifications.notifyLogged('updateAvatar', {                                                          // 148
							username: user.username                                                                                         // 148
						});                                                                                                              // 148
					}, 500);                                                                                                          // 149
				}));                                                                                                               // 150
				rs.pipe(ws);                                                                                                       // 151
...
sync = function () {                                                                                                   // 195
	function sync() {                                                                                                     // 195
		if (RocketChat.settings.get('LDAP_Enable') !== true) {                                                               // 196
			return;                                                                                                             // 197
		}                                                                                                                    // 198
                                                                                                                       //
		var ldap = new LDAP();                                                                                               // 200
                                                                                                                       //
		try {                                                                                                                // 202
			ldap.connectSync();                                                                                                 // 203
			var users = RocketChat.models.Users.findLDAPUsers();                                                                // 205
                                                                                                                       //
			if (RocketChat.settings.get('LDAP_Import_Users') === true && RocketChat.settings.get('LDAP_Username_Field') !== '') {
				var ldapUsers = ldap.searchUsersSync('*');                                                                         // 208
				ldapUsers.forEach(function (ldapUser) {                                                                            // 209
					var username = slug(getLdapUsername(ldapUser)); // Look to see if user already exists                             // 210
                                                                                                                       //
					var userQuery = {                                                                                                 // 212
						username: username                                                                                               // 213
					};                                                                                                                // 212
					logger.debug('userQuery', userQuery);                                                                             // 216
					var user = Meteor.users.findOne(userQuery);                                                                       // 218
                                                                                                                       //
					if (!user) {                                                                                                      // 220
						addLdapUser(ldapUser, username);                                                                                 // 221
					} else if (user.ldap !== true && RocketChat.settings.get('LDAP_Merge_Existing_Users') === true) {                 // 222
						syncUserData(user, ldapUser);                                                                                    // 223
					}                                                                                                                 // 224
				});                                                                                                                // 225
			}                                                                                                                   // 226
                                                                                                                       //
			users.forEach(function (user) {                                                                                     // 228
				var ldapUser = void 0;                                                                                             // 229
                                                                                                                       //
				if (user.services && user.services.ldap && user.services.ldap.id) {                                                // 231
					ldapUser = ldap.getUserByIdSync(user.services.ldap.id, user.services.ldap.idAttribute);                           // 232
				} else {                                                                                                           // 233
					ldapUser = ldap.getUserByUsernameSync(user.username);                                                             // 234
				}                                                                                                                  // 235
                                                                                                                       //
				if (ldapUser) {                                                                                                    // 237
					syncUserData(user, ldapUser);                                                                                     // 238
				} else {                                                                                                           // 239
					logger.info('Can\'t sync user', user.username);                                                                   // 240
				}                                                                                                                  // 241
			});                                                                                                                 // 242
		} catch (error) {                                                                                                    // 243
	...
	return sync;                                                                                                          // 195
                                                                                                                       ...
                                                                                                                       //
var interval = void 0;                                                                                                 // 252
var timeout = void 0;                                                                                                  // 253
RocketChat.settings.get('LDAP_Sync_User_Data', function (key, value) {                                                 // 255
	Meteor.clearInterval(interval);                                                                                       // 256
	Meteor.clearTimeout(timeout);                                                                                         // 257
                                                                                                                       //
	if (value === true) {                                                                                                 // 259
		logger.info('Enabling LDAP user sync');                                                                              // 260
		interval = Meteor.setInterval(sync, 1000 * 60 * 60);                                                                 // 261
		timeout = Meteor.setTimeout(function () {                                                                            // 262
			sync();                                                                                                             // 263
		}, 1000 * 60 * 10);                                                                                                  // 264
	} else {                                                                                                              // 265
		logger.info('Disabling LDAP user sync');                                                                             // 266
	}                                                                                                                     // 267
});                                                                                                                    // 268
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
